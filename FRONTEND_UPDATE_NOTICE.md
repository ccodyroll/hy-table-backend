...
백엔드의 `notes` 필드 파싱 로직이 개선되어 **영어 형식(AM/PM)과 한국어 오전/오후 형식도 지원**합니다.
...
- 한국어 오전/오후: `"월요일 오후 5-7시 안돼"`, `"월 오전 9-10시"` ✅ (새로 추가)
...
- ✅ `"월요일 오후 5-7시 안돼"` → `"blocked_MON_1700_1900"` (새로 추가)
- ✅ `"월 오전 9-10시"` → `"blocked_MON_0900_1000"` (새로 추가)
- ✅ `"월요일 15:00-20:00 불가"` → `"blocked_MON_1500_2000"`
